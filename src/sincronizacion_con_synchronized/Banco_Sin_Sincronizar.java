package sincronizacion_con_synchronized;
...
class Banco {
	...
	public synchronized void transferencia(int cuenta_origen, int cuenta_destino, double cantidad)
		...
		while (this.cuentas[cuenta_origen] < cantidad) {
			System.err
					.println("HILO " + Thread.currentThread().getName() + " a la espera, en la cuenta " + cuenta_origen
							+ " con saldo actual: " + cuentas[cuenta_origen] + ", queriendo enviar: " + cantidad);

			// a la espera, metodo de Object
			wait();
		}
		...
		// avisar a los hilos en espera que revisen si tiene que despertar o seguir
		// esperando
